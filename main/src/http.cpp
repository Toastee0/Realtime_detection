    ...
    if (!isInitialized) {
        ...
        std::thread([]() {
            while (true) {
                ...
                std::string result = model_detector(global_model, global_camera, i);
                MA_LOGI(TAG, "Model detector result:\n%s", result.c_str());

                i++;

                auto elapsed = std::chrono::steady_clock::now() - start;
                auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count();
                MA_LOGI(TAG, "Cycle duration: %ldms", ms);

                // Espera 5 segundos entre detecciones
