        ...
        for (auto result : _results) {
            ...
            if(class_name == "no_helmet") {
                if (!should_save_image) {
                    // Preparar imagen para guardar solo si encontramos no_helmet
                ...
                // Dibujar solo si vamos a guardar la imagen
                if (should_save_image) {
                    cv::rectangle(output_image, cv::Point(x1, y1), cv::Point(x2, y2), 
                                ColorPalette::getColor(result.target), 3, 8, 0);
                    cv::putText(output_image, class_name, cv::Point(x1, y1 - 10), 
                                cv::FONT_HERSHEY_SIMPLEX, 0.6, 
                                ColorPalette::getColor(result.target), 2, cv::LINE_AA);
                }
            }
        }

        // Guardar imagen solo si se detectó no_helmet
        if (should_save_image) {
            std::string filename2 = "image_detected_" + std::to_string(i) + ".jpg";
            try {
                if (cv::imwrite(filename2, output_image)) {
                    rest += "| Image saved: " + filename2;
                } else {
                    MA_LOGE(TAG, "Failed to save image: %s", filename2.c_str());
                }
            } catch (const cv::Exception& e) {
                MA_LOGE(TAG, "OpenCV exception when saving image: %s", e.what());
            }
        }
    }
